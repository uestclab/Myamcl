...
#include "prosper_underpan/CarPoseInfo.h"
...
//0327
#include "matlab/CSMmap.h"
#include "matlab/EKF.h"
#include "matlab/GlobalLocalization.h"
...
static double
angle_diff(double a, double b) //(new , old)
{
    double d1, d2;
    a = normalize(a);
    b = normalize(b);
...
static const std::string scan_topic_ = "scan"; //scan -- LQ
...
class AmclNode
{
  ...
  private:
    ...
    std::vector<AMCLLaser *> lasers_; // 在AMCL中的laser数据格式 -- LQ
    ...
    //debug
    LoadMap *GaussianMap_;
    EKF *EKF_;
    GlobalLocalization *GlobalLocal_;
	...
	boost::thread* RotateCarThread_; // 0414_thread
	boost::condition_variable rotate_cond; // 0414_thread
	boost::mutex rotate_mutex_; //0414_thread
	bool runRotate;
	void rotateCarCondition(); // 0414_thread
	void WakeupRotate(); // 0414_thread
	void rotateCarThreadForever();
	void DestroyThread();
...
int main(int argc, char **argv)
{
    ...
    string info_log = path + name + "_info_";
    google::SetLogDestination(google::INFO, info_log.c_str());

    string warning_log = path + name + "_warning_";
    google::SetLogDestination(google::WARNING, warning_log.c_str());

    string error_log = path + name + "_error_";
    google::SetLogDestination(google::ERROR, error_log.c_str());

    string fatal_log = path + name + "_fatal_";
    google::SetLogDestination(google::FATAL, fatal_log.c_str());

    ros::init(argc, argv, "amcl");
    ros::NodeHandle nh;
    //ProfilerStart("/home/liqing/amcl_study/amcl_ws/src/navigation-indigo-devel/amcl/log/out.prof");
...
AmclNode::AmclNode() : sent_first_transform_(false),
                       ...
                       GlobalLocal_(NULL)
{
    ...
    //debug
    car_pose_pub = nh_.advertise<prosper_underpan::CarPoseInfo>("CarPoseUpdate", 10);
    pcl_pub = nh_.advertise<sensor_msgs::PointCloud2>("pcl_output", 10);
    //end debug
    ...
    if (use_map_topic_)
    {
        ...
        LOG(INFO) << "use_map_topic_...<xiaoafei>";
    }
    else
    {
        requestMap();
        LOG(INFO) << "no use_map_topic_...<xiaoafei>";
    }
	...
	runRotate = false; //0414_thread
	RotateCarThread_ = new boost::thread(boost::bind(&AmclNode::rotateCarThreadForever, this)); // 0414_thread
}


void AmclNode::rotateCarThreadForever()
{
	cout << "Thread rotateCarThreadForver started ------" << endl;
	try{
		for(;;){
			std::this_thread::sleep_for(std::chrono::milliseconds(100));
			{
				boost::unique_lock<boost::mutex> lock(rotate_mutex_);
				boost::this_thread::interruption_point(); 
				//cout << "Run Thread rotateCarThreadForever()----------------" << endl; 
			}
		}
	}
	catch(...){
		cout << "Interrupt exception was thrown ----" << endl;
	}
	cout << "Leave Thread ----" << endl;
}

void AmclNode::DestroyThread()
{
	cout << "Call DestroyThread() ---- " << endl;
	RotateCarThread_->interrupt();
	RotateCarThread_->join();
	cout << "Leave DestroyThread() ---- " << endl;
}


// 0414_thread _ start
void AmclNode::rotateCarCondition()
{
	while(true){
		boost::unique_lock<boost::mutex> lock(rotate_mutex_);
		while(!runRotate){
			rotate_cond.wait(lock);
		}
		runRotate = false;
	}
}

void AmclNode::WakeupRotate()
{
	//boost::unique_lock<boost::mutex> lock(rotate_mutex_);
	//lock.lock();
	runRotate = true;
	cout << "call WakeupRotate() ----------------------------------------------------------------" << endl;
	rotate_cond.notify_one();
	//lock.unlock();
}
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
    ...
    pf_init(pf_, pf_init_pose_mean, pf_init_pose_cov, 0);
    pf_init_ = false;
...
void AmclNode::mapReceived(const nav_msgs::OccupancyGridConstPtr &msg)
{
    ...
    //  LOG(WARNING)<<"mapReceived...<xiaoafei>";
...
void AmclNode::handleMapMessage(const nav_msgs::OccupancyGrid &msg)
{
    ...
    // LOG(WARNING)<<"handleMapMessage...<xiaoafei>";
    ...
    applyInitialPose(); // 根据map读入的中心位置作为均值，配合初始化粒子滤波器开始撒点 -- LQ
}
...
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr &laser_scan)
{
    ...
    if (Kidnap == true && GlobalLocal_ == NULL)
    {
        Vector3d init = Vector3d::Zero();
        if (GaussianMap_ != NULL)
        {
            GlobalLocal_ = new GlobalLocalization(init, 0.05, GaussianMap_->loadmap_, GaussianMap_->origin_x, GaussianMap_->origin_y);
            if(Global)
            {
                GlobalLocal_->GiveWholeMapRandom(GaussianMap_->map_, 200);
            }
            else
            {
                GlobalLocal_->InitPoseLocalization(GaussianMap_->map_, init_pose_, 3, 10, 10000);
            }
        }
    }

	if(GlobalLocal_->particles.size() == 0 && ekf == 0)
	{
		cout << "No particles , Global Localization is failture ! " << endl;
		return;
	}

    if (Kidnap && GlobalLocal_ != NULL)
    {
        int num_p = GlobalLocal_->particles.size();
        geometry_msgs::PoseArray cloud_msg;
        cloud_msg.header.stamp = ros::Time::now();
        cloud_msg.header.frame_id = global_frame_id_;
        cloud_msg.poses.resize(num_p);
        for (int i = 0; i < num_p; i++)
        {
            Vector3d pose = GlobalLocal_->particles[i].first;
            tf::poseTFToMsg(tf::Pose(tf::createQuaternionFromYaw(pose(2)),
                                     tf::Vector3(pose(0),
                                                 pose(1), 0)),
                            cloud_msg.poses[i]);
        }
        particlecloud_pub_.publish(cloud_msg); // 输出ros 当前所有粒子的状态 -- LQ
    }
    ...
    if (frame_to_laser_.find(laser_scan->header.frame_id) == frame_to_laser_.end()) // 第一帧才进 -- LQ
    {
        ...
        try
        {
            this->tf_->transformPose(base_frame_id_, ident, laser_pose); //ident = 从odom原点看base_link,获取当前laser数据在base_link坐标系下的坐标？ -- LQ
        }
        catch (tf::TransformException &e)
        {
    ...
    {
        std::cout << "getOdomPose() failure --------test" << std::endl;
    ...
    if (Kidnap)
    {
        ...
        else
        {
            ...
            Cur_odom(1) = pose.v[1];
            Cur_odom(2) = pose.v[2];
            if (GlobalLocal_ != NULL)
            {
                GlobalLocal_->predict_move(GaussianMap_->map_, Pre_odom, Cur_odom);
                ...
                if (laser_count % 40 == 0)
                {
                    ...
                    double delta = laser_scan->angle_increment;
                    double initTheta = laser_scan->angle_min;
                    for (int i = 0; i < range_count; i++)
                    {
                        laserRange[i] = laser_scan->ranges[i];
                        theta[i] = initTheta + i * delta;
                    }
                    GlobalLocal_->observe_ROSLikelihood(laserRange, theta, range_count, laser_max_range_, 2);

                    double cvx = 0, cvy = 0, cvtheta = 0;
                    GlobalLocal_->cov(&cvx, &cvy, &cvtheta);
                    if (cvx + cvy < 0.0025)
                    {
                        Vector3d bestPose = GlobalLocal_->particles[0].first;
						//cout << "before SearchBestAlign Pose = " << bestPose << endl;
                        Matrix3d cov;
                        double bestscore = GlobalLocal_->SearchBestAlign(&bestPose, laserRange, theta, range_count,
                                                                         0.5, 0.5, 0.1, 25 * M_PI / 180, 2 * M_PI / 180,
                                                                         true, &cov, laser_max_range_);
						//cout << "after SearchBestAlign BestPose = " << bestPose << "---- bestscore = " << bestscore << endl;
                        if (bestscore < 0)
                            GlobalLocal_->GiveWholeMapRandom(GaussianMap_->map_, 200);
                        else
                        {
                            if (EKF_ == NULL)
                            {
                                Eigen::Matrix3d initCov = Eigen::Matrix3d::Identity() * 0.00001;
                                initPose(0) = bestPose(0);
                                initPose(1) = bestPose(1);
                                initPose(2) = bestPose(2);
                                EKF_ = new EKF(initPose, initCov, GaussianMap_->loadmap_, 0.05, GaussianMap_->origin_x, GaussianMap_->origin_y);
                                Kidnap = false;
                                ekf = 1;
								DestroyThread();
                            }
                    ...
                    {
                        GlobalLocal_->observe_ROSBeamModel(laserRange, theta, range_count, GaussianMap_->map_, 10);
                        float range = sqrt(cvx + cvy) > 0.5 ? 0.5 : sqrt(cvx + cvy);
                        float angle = (20.0 / 180.0) * M_PI > sqrt(cvtheta) ? sqrt(cvtheta) : (20.0 / 180.0) * M_PI;
                        GlobalLocal_->GenerateRandomPose(range, angle);
                    }
    ...
    if (ekf == 1 && !Kidnap)
    {
        ...
        // 虚拟frame test
        // GlobalLocal_->test_ROSBeamModel(laserRange, theta, range_count, GaussianMap_->map_, 10, pose_mean);
        // pcl_pub.publish(GlobalLocal_->output);

        // publish
        tf::Stamped<tf::Pose> odom_to_map;
        try
        {
            tf::Transform tmp_tf(tf::createQuaternionFromYaw(pose_mean(2)), //tmp_tf = 从map原点看base_link的位置 -- LQ
                                 tf::Vector3(pose_mean(0),
                                             pose_mean(1),
                                             0.0));
            tf::Stamped<tf::Pose> tmp_tf_stamped(tmp_tf.inverse(), //tmp_tf.inverse() = 以为Mp为原点，map原点的位置，就是在base_link坐标系下map 原点的位置 -- LQ
                                                 laser_scan->header.stamp,
                                                 base_frame_id_);
            this->tf_->transformPose(odom_frame_id_, //进行 base_link坐标系下的点转换到odom坐标系，也就是把map原点转换到odom坐标系下，等于从odom原点看map原点的位置。放到latest_tf_变量里面。
                                     tmp_tf_stamped,
                                     odom_to_map);
        }
        ...
        tf::StampedTransform tmp_tf_stamped(latest_tf_.inverse(), //发布tf转换关系，latest_tf_.inverse() 得到的是从map原点看odom原点的位置，也就是 odom->map的转换关系
                                            transform_expiration,
                                            global_frame_id_, odom_frame_id_);
        this->tfb_->sendTransform(tmp_tf_stamped);

        //std::cout << "run-time = "<<ros::Time::now() - time1<<std::endl;

        prosper_underpan::CarPoseInfo car_pose_info;
        car_pose_info.uid = map_uid;
        car_pose_info.x = pose_mean(0);
        car_pose_info.y = pose_mean(1);
        car_pose_info.th = pose_mean(2) * 180 / M_PI;
        car_pose_pub.publish(car_pose_info);
    ...
    //----------------------------------------------------below is old code , not run now!! -----------------------------------------------------------------------
    cout << "----------------------------------------------below is old code , not run now!!---------------------------------------------" << endl;
    //end_debug
    ...
    {
        //debug
        Pre_odom(0) = pf_odom_pose_.v[0];
        Pre_odom(1) = pf_odom_pose_.v[1];
        Pre_odom(2) = pf_odom_pose_.v[2];
        ...
        update = update || m_force_update || laser_count > 2;
        m_force_update = false;
        //update = true;
        // Set the laser update flags
        if (update) // 足够的移动或旋转触发粒子状态更新 -- LQ
        {
            std::cout << "lasers_update_.size() =   " << lasers_update_.size() << " ---------test" << std::endl;
        ...
        if (laser_count > 30 && initLocalization)
        {
            ...
            {
                laser_count = 0;
                tf::Pose relocalizePose = latest_tf_.inverse() * latest_odom_pose_; //应用最后一次publish的变换结合最新的odom数据，得到当前baselink最新的map坐标 -- LQ
                Relocalize(relocalizePose);
            }
        }
    ...
    else if (pf_init_ && lasers_update_[laser_index]) // 周期性的处理 , 需要小车移动足够多的距离或旋转才触发-- LQ
    {
        //printf("pose\n");
        ...
        odom_->UpdateAction(pf_, (AMCLSensorData *)&odata); // 根据当前的odom数据对粒子的运动进行预测估计 -- LQ
    ...
    if (lasers_update_[laser_index])
    {
        ...
        lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData *)&ldata); // 根据当前的laser对粒子进行weight更新 -- LQ

        std::cout << "After time = " << (ros::Time::now() - time).toSec() << std::endl;
        ...
        {
            pf_update_resample(pf_); // 重采样
            resampled = true;
        }

        pf_sample_set_t *set = pf_->sets + pf_->current_set;

        //std::cout << "Num samples = " << set->sample_count << " cluster_count = " << set->cluster_count << "cluster_max_count = " << set->cluster_max_count << std::endl;
 ...
 */   //if
    ...
    if (resampled || force_publication) // 当重采样进行后，触发选取最佳粒子输出ros -- LQ
    {
        ...
        std::cout << "Max weight = " << max_weight << " --------------------------------Pose = " << hyps[max_weight_hyp].pf_pose_mean.v[0] << " , " << hyps[max_weight_hyp].pf_pose_mean.v[1] << " , "
                  << hyps[max_weight_hyp].pf_pose_mean.v[2] << std::endl;

        if (max_weight > 0.0)
        {
            //debug
            pf_sample_set_t *set1 = pf_->sets + pf_->current_set;
            if (set1->sample_count < 120)
            {
                initPose(0) = hyps[max_weight_hyp].pf_pose_mean.v[0];
                initPose(1) = hyps[max_weight_hyp].pf_pose_mean.v[1];
                initPose(2) = hyps[max_weight_hyp].pf_pose_mean.v[2];
                ekf = 1;
                if (EKF_ == NULL)
                {
                    Eigen::Matrix3d initCov = Eigen::Matrix3d::Identity() * 0.00001;
                    EKF_ = new EKF(initPose, initCov, GaussianMap_->loadmap_, 0.05, GaussianMap_->origin_x, GaussianMap_->origin_y);
                    std::cout << "if(EKF_ == NULL)" << std::endl;
                }
            }
            //end_debug
            ...
            try
            {
                tf::Transform tmp_tf(tf::createQuaternionFromYaw(hyps[max_weight_hyp].pf_pose_mean.v[2]), //tmp_tf = 从map原点看base_link的位置 -- LQ
...
void AmclNode::initialPoseReceived(const geometry_msgs::PoseWithCovarianceStampedConstPtr &msg) //外部在rviz上重新选取初始位置触发 -- LQ
{
...
void AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped &msg)
{
    ...
    applyInitialPose();
    std::cout << "AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)" << std::endl;
}
...
void AmclNode::applyInitialPose()
{
    ...
    {
        pf_init(pf_, initial_pose_hyp_->pf_pose_mean, initial_pose_hyp_->pf_pose_cov, 0);
        pf_init_ = false;
...
//debug
void AmclNode::Relocalize(tf::Pose &relocalizePose)
{
    boost::recursive_mutex::scoped_lock prl(configuration_mutex_);
    ...
    pf_matrix_t pf_init_pose_cov = pf_matrix_zero();
    pf_init_pose_cov.m[0][0] = init_cov_[0] / (8 * 24);
    pf_init_pose_cov.m[1][1] = init_cov_[1] / (8 * 24);
    pf_init_pose_cov.m[2][2] = init_cov_[2] / (8 * 24);
    ...
    if (initial_pose_hyp_ != NULL && map_ != NULL)
    {
        pf_init(pf_, initial_pose_hyp_->pf_pose_mean, initial_pose_hyp_->pf_pose_cov, 1);
        pf_init_ = false;
    ...
    std::cout << "AmclNode::Relocalize(tf::Pose relocalizePose)" << std::endl;
}
...
AmclNode::changeMap()
{
    ...
    imageOri = imread("/home/liqing/amcl_study/amcl_ws/src/yaml_test/map.pgm", IMREAD_GRAYSCALE);
    map->size_x = imageOri.cols;
    map->size_y = imageOri.rows;
    map->scale = 0.05;
    ...
    for (size_t x = 0; x < imageOri.rows; ++x)
    {
        ...
        {
            occ = (255 - *(p + y)) / 255.0;
            int i = (imageOri.rows - x - 1) * map->size_x + y;
            if (occ > occ_th)
                map->cells[i].occ_state = +1;
            else if (occ < free_th)
                map->cells[i].occ_state = -1;
...
/*read map xiaoafei  amcl粒子滤波 读取图片*/
map_t *
AmclNode::readMap()
{
    LOG(WARNING) << "readMap begin...<xiaoafei>";
    ...
    map_path = "/tmp/CarStatus.yaml";

    ReadCarStatusFromYaml(map_path, &uid, &x, &y, &th, &map_file, &map_yaml);

	// read init pose from file , and call GlobalLocalization interface; 
    init_pose_[0] = x;
    init_pose_[1] = y;
    init_pose_[2] = th;

    map_uid = uid;
    // LOG(WARNING)<<"readMap..."<<map_png << "  "<<map_yaml;
    ...
    map->origin_x = -xorigin + (map->size_x / 2) * map->scale;
    map->origin_y = -yorigin + (map->size_y / 2) * map->scale;
    ...
    if (strstr(map_file.c_str(), ".png") != NULL)
    {
        ...
        LOG(WARNING) << "strstr(map_file.c_str() --- PNG";
    }
    else if (strstr(map_file.c_str(), ".pgm") != NULL)
    {
        ...
        // LOG(WARNING)<<"strstr(map_file.c_str() --- PGM";
    }

    std::cout << "AmclNode::readMap end  " << std::endl;
    return map;
}
