...
/*
  ...
  std::ifstream yaml_file("/home/liqing/amcl_study/amcl_ws/src/yaml_test/map.yaml");
	...
	doc["free_thresh"] >> free_th;
	  ...
	  Mat imageOri;
	  imageOri = imread(mapfname,IMREAD_GRAYSCALE);
...
void AmclNode::WriteMapInfoToYaml(uint32_t uid,int floor,double origin_x,double origin_y)
{
    ...
    {
        std::cout<<"void WriteMappingInfoToYaml..."<<std::endl;
        
        YAML::Emitter out(yaml_file);
        out << YAML::BeginMap;
        out << YAML::Key << "uid" << YAML::Value << uid;
        out << YAML::Key << "floor" << YAML::Value << floor;
		out << YAML::Key << "image" << "/home/liqing/bagfiles/map/map.pgm";
		out << YAML::Key << "resolution" << YAML::Value << 0.05;
		out << YAML::Key << "xorigin" << YAML::Value << -19.47783432006836;
        out << YAML::Key << "yorigin" << YAML::Value << -5.01034851074219;
